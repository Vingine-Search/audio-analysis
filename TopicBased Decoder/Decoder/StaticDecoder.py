import sys
sys.path.append("../FeaturesExtraction")
import os.path
import cntk
import numpy as np
import scipy.sparse
...
# this function is used to load the feature file and return the feature vector and utterance name
def load_parameters(feat_file: str):
    """This function loads a feature file and returns the feature vector and utterance name."""
    # read the feature file
    feat = read_htk_user_feat(feat_file)
    # get the utterance name
    utterance_name = os.path.splitext(os.path.basename(feat_file))[0]
    return feat, utterance_name
...
def main():
    ...
    parser.add_argument('-feat', '--feat', help='Feature file name', required=True, default=None)
    ...
    with open(args.trn, 'w', buffering=1) as ftrn:
        try:
            ...
            for i in range(max_seconds):
                activations = z.eval(feats[i * 99: (i*99 + 99), :].astype('f'))[0]
            ...
            activations = z.eval(feats[max_seconds * 99 : , :].astype('f'))[0]
            hypothesis = fst.decode(activations, beam_width=args.beam_width, lmweight=args.lmweight)
            words = [
                x for x in
                map(operator.itemgetter(1), hypothesis)
...
class token_manager:
    ...
    def commit_active_tokens(self):
        ...
        #### equivalent to putting the current active_tokens into the frontiers list. ####
